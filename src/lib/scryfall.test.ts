...
describe("scryfall client", () => {
  ...
  it("fetches random cards and named cards with error handling", async () => {
    ...
    await expect(getCardByName("Missing Card")).rejects.toThrow(/Card not found/i);
  });

  it("formats card display helpers correctly", async () => {
    const { getCardImage, isDoubleFacedCard, getCardFaceDetails, getRarityColor, formatManaSymbols } =
      await loadModule();
  ...
  it("fetches rulings with caching and error handling", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(
        mockResponse({ data: [{ oracle_id: "1", source: "wotc", published_at: "2020-01-01", comment: "a" }] }),
      )
      .mockResolvedValueOnce(mockResponse({ object: "error" }, 500))
  ...
  it("rejects when the request queue is full", async () => {
    ...
    const queued = Array.from({ length: 50 }, (_, index) => searchCards(`t:card-${index}`));

    await expect(searchCards("t:overflow")).rejects.toThrow(/Too many pending requests/i);

    resolveFetch?.(
      mockResponse({ object: "list", total_cards: 0, has_more: false, data: [] }),
    );
