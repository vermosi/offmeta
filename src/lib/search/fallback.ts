...
export const PRETRANSLATED: Record<string, string> = {
  ...
  'green ramp spells that search for lands':
    'c:g otag:mana-ramp o:"search your library" o:"basic land"',
  'elf tribal payoffs for commander':
    't:elf f:commander (o:"elf" o:"you control" or o:"elf" o:"+1/+1")',
  'creatures that make token creatures when an opponent takes an action':
    't:creature o:"whenever" o:"opponent" o:"create" o:"token"',
  'mana rocks that cost 2': 't:artifact mv=2 (o:"add" o:"{")',
  'cards that double etb effects':
    'o:"enters the battlefield" o:"triggers an additional time"',
  'cards that give haste': '(o:"gain haste" or o:"gains haste" or kw:haste)',
  'cards that give flash': '(o:"gain flash" or o:"gains flash" or kw:flash)',
  'utility lands for commander in esper under $5':
    't:land -t:basic id<=wub f:commander usd<5',
  // Archetypes
  'equipment or auras that give bonuses to equipped or enchanted permanent for commander':
    '(t:equipment or t:aura) (o:"equipped creature" or o:"enchanted creature") f:commander',
  'creatures that deal damage or drain life when a creature dies for commander':
    't:creature o:"whenever" (o:"dies" or o:"creature dies") (o:"deals" or o:"lose" or o:"drain") f:commander',
  'blue red instants or sorceries that reward casting spells for commander':
    'id<=ur (t:instant or t:sorcery or o:"whenever you cast") f:commander',
  'selesnya cards that create creature tokens for commander':
    'id<=gw o:"create" o:"token" f:commander',
  'black cards that return creatures from graveyard to battlefield for commander':
    'c:b o:"return" o:"from" o:"graveyard" o:"battlefield" f:commander',
  'white cards that restrict or tax opponents for commander':
    'c:w (o:"opponents" or o:"each opponent") (o:"pay" or o:"can\'t" or o:"cost" o:"more") f:commander',
  'simic cards that let all players draw cards or gain mana for commander':
    'id<=gu (o:"each player" or o:"all players") (o:"draw" or o:"add") f:commander',
  'cards that mill opponents or put cards from library into graveyard for commander':
    '(o:"mill" or (o:"library" o:"graveyard")) f:commander',
  'landfall cards legal in commander': 'otag:landfall f:commander',
  'orzhov cards that gain life or care about lifegain triggers for commander':
    'id<=wb (o:"gain" o:"life" or o:"whenever" o:"life") f:commander',
  'selesnya creatures that add or care about +1/+1 counters for commander':
    'id<=gw t:creature o:"+1/+1 counter" f:commander',
  'azorius cards that exile and return permanents or have etb effects for commander':
    'id<=wu (o:"exile" o:"return" or o:"enters the battlefield") f:commander',
  'izzet cards that make all players discard and draw for commander':
    'id<=ur o:"each player" o:"discard" o:"draw" f:commander',
  'golgari cards that recur from graveyard or benefit from creatures dying for commander':
    'id<=bg (o:"from your graveyard" or o:"whenever" o:"dies") f:commander',
  'planeswalkers or cards that proliferate or protect planeswalkers for commander':
    '(t:planeswalker or o:"proliferate" or o:"planeswalker" o:"protect") f:commander',
  'selesnya enchantments or creatures that draw cards when enchantments enter for commander':
    'id<=gw (t:enchantment or t:creature) o:"enchantment" o:"draw" f:commander',
  'simic creatures with infect or proliferate for commander':
    'id<=gu (kw:infect or o:"proliferate") f:commander',
  'treasure token cards legal in commander':
    'o:"treasure" o:"token" f:commander',
  'izzet cards with storm or that copy spells or reduce spell costs for commander':
    'id<=ur (kw:storm or o:"copy" o:"spell" or o:"costs" o:"less") f:commander',
  'chaos cards legal in commander':
    '(o:"coin" or o:"random" or o:"chaos") f:commander',
  'tribal lords legal in commander': '(otag:lord or otag:anthem) f:commander',
  'azorius enchantments or artifacts that prevent attacks or tax attackers for commander':
    'id<=wu (t:enchantment or t:artifact) (o:"can\'t attack" or o:"attacks" o:"pay") f:commander',
  'azorius counterspells or board wipes or removal for commander':
    'id<=wu (otag:counter or otag:boardwipe or otag:removal) f:commander',
  'artifacts that tap for blue': 't:artifact o:"add" o:"{U}"',
  'lands that add any color': 't:land o:"add" o:"any color"',
  'mana fixing':
    '(t:land or t:artifact) (o:"add" or o:"search" or o:"any color")',
...
const SLANG_MAP: Record<string, string> = {
  ...
  'combat evasion':
    '(kw:flying or kw:menace or kw:trample or kw:unblockable or kw:shadow or kw:fear or kw:intimidate or kw:skulk or o:"can\'t be blocked")',
  'mass combat evasion':
    '(kw:flying or kw:menace or kw:trample or o:"can\'t be blocked") o:"creatures you control"',
  ...
  'protection spells':
    '(kw:hexproof or kw:indestructible or o:"protection from")',
  'protection spell':
    '(kw:hexproof or kw:indestructible or o:"protection from")',
...
const MANA_PRODUCTION_PATTERNS: Array<{
  regex: RegExp;
  syntax: string | ((m: RegExpMatchArray) => string);
}> = [
  // "tap for <color>" e.g. "tap for blue", "tap for any color"
  {
    regex:
      /\b(?:tap for|produce|generate|add)\s+(white|blue|black|red|green|colorless|any(?:\s+color)?)\b/i,
  ...
  {
    regex:
      /\b(?:produce|generate|add|tap for)\s+(\d+)\s*(?:or more\s+)?mana\b/i,
    syntax: 'o:"add"',
  },
  // "produce/generate/add mana" (no number)
  {
    regex: /\b(?:produce|generate|add|tap for)\s+mana\b/i,
    syntax: 'o:"add" o:"{"',
  },
  ...
  {
    regex: /\b(?:add\s+)?any\s+color(?:\s+of\s+mana)?\b/i,
    syntax: 'o:"add" o:"any color"',
  },
  // "make/create treasure token(s)"
  {
    regex: /\b(?:make|create|generate)\s+treasure\s+tokens?\b/i,
    syntax: 'o:"create" o:"Treasure token"',
  },
];
...
export function isLikelyCardName(query: string): boolean {
  ...
  const hasSearchKeywords =
    /\b(with|that|under|below|above|less|more|cheap|budget|from|legal|commander|deck|spells?|cards?|creatures?|artifacts?|enchantments?|lands?|instants?|sorcery|sorceries|produce|generate|create|make|draw|search|find|tap for|best|good|great|top|payoffs?|synerg(?:y|ies)|released|after|before|since|until|mana|rocks?|wipes?|board|ramp|removal)\b/i.test(
      trimmed,
    );
  ...
  const allCapitalized = words.every(
    (w) => /^[A-Z]/.test(w) || /^(of|the|and|to|in|for|a|an)$/i.test(w),
  );

  // Single-word MTG terms that are NOT card names
  const singleWordMtgTerms =
    /^(flying|trample|haste|deathtouch|lifelink|vigilance|reach|menace|flash|hexproof|indestructible|ward|defender|infect|prowess|cascade|storm|ramp|removal|mill|blink|tokens?|sacrifice|voltron|aristocrats|reanimation|lifegain|tutor|counterspell|boardwipe|flicker|cycling|landfall|scry|proliferate|populate|red|blue|green|white|black|colorless|multicolor|mono|tribal|burn|bounce|copy|clone|theft|discard|anthem|lord|stax|hatebear|aggro|combo|midrange|tempo|control|prison|equipment|aura|ping)$/i;
  if (words.length === 1 && singleWordMtgTerms.test(trimmed)) return false;
  if (words.length === 1 && !singleWordMtgTerms.test(trimmed) && allCapitalized)
    return true;
...
export function extractCardNameCandidate(query: string): string | null {
  ...
  if (words.length >= 1 && words.length <= 4) {
    const hasKeywords =
      /\b(with|that|under|below|above|less|more|cheap|budget|from|legal|commander|deck|spells?|create|make|search|find|tap|produce|generate)\b/i.test(
        trimmed,
      );
...
export function buildClientFallbackQuery(naturalQuery: string): string {
  ...
  const searchForMatch = residual.match(
    /\b(?:search|find|tutor|look)\s+(?:for\s+)?(?:an?\s+)?(artifact|creature|enchantment|instant|sorcery|land|equipment)\s*(?:from|in)?\s*(?:my|your|the)?\s*(?:library|deck)?\b/i,
  );
  if (searchForMatch) {
  ...
  const manaValueMatch = residual.match(
    /\b(?:under|less than|below)\s+(\d+)\s+mana\b/i,
  );
  if (manaValueMatch) {
    ...
    const mvOrLessMatch = residual.match(
      /\b(?:costs?\s+)?(\d+)\s+(?:mana\s+)?or\s+less\b/i,
    );
    if (mvOrLessMatch) {
  ...
  const priceMatch = residual.match(
    /\b(?:under|less than|below)\s+\$?\s*(\d+(?:\.\d+)?)\s*(?:dollars?)?\b/i,
  );
  if (priceMatch) {
  ...
  const yearMatch = residual.match(
    /\b(?:released?\s+)?(?:after|since)\s+((?:19|20)\d{2})\b/i,
  );
  if (yearMatch) {
    parts.push(`year>${yearMatch[1]}`);
    residual = residual.replace(yearMatch[0], ' ').trim();
  } else {
    const yearBeforeMatch = residual.match(
      /\b(?:released?\s+)?(?:before|until)\s+((?:19|20)\d{2})\b/i,
    );
    if (yearBeforeMatch) {
  ...
  const isSingleContentWord =
    residualWords.length === 1 && /^[a-z][a-z'-]{3,}$/i.test(residualWords[0]);
