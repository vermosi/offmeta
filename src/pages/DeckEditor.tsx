...
export default function DeckEditor() {
  ...
  useEffect(() => {
    ...
    const importCards = async () => {
      ...
      toast({
        ...
        description: t('deckEditor.cardsImportedDesc').replace(
          '{count}',
          String(state.importCards!.length),
        ),
      });
  ...
  const handleRecategorizeAll = useCallback(async () => {
    ...
    try {
      ...
      if (error || !data?.categories) {
        toast({
          title: t('deckEditor.categorizeFailed'),
          variant: 'destructive',
        });
        return;
      }
      ...
      toast({
        ...
        description: t('deckEditor.categorizeSuccessDesc').replace(
          '{count}',
          String(cardNames.length),
        ),
      });
    } catch {
  ...
  const handleSuggest = useCallback(async () => {
    ...
    try {
      ...
      if (error || !data?.suggestions) {
        toast({
          title: t('deckEditor.suggestions.failed'),
          variant: 'destructive',
        });
        return;
      }
  ...
  const handleRemoveByName = useCallback(
    (cardName: string) => {
      const card = cards.find(
        (c) => c.card_name === cardName && c.board !== 'maybeboard',
      );
  ...
  const handleSetCompanion = useCallback(
    (cardId: string, isCompanion: boolean) => {
      ...
      if (isCompanion) {
        for (const c of cards) {
          if (c.is_companion && c.id !== cardId)
            updateCard.mutate({ id: c.id, is_companion: false });
        }
  ...
  const handleTogglePublic = useCallback(() => {
    ...
    toast({
      title: deck.is_public
        ? t('deckExport.nowPrivate')
        : t('deckExport.nowPublic'),
      description: deck.is_public
        ? t('deckExport.nowPrivateDesc')
        : t('deckExport.nowPublicDesc'),
    });
    ...
    <div className="border-b border-border bg-card">
      <div className="px-4 py-3 flex items-center gap-3">
        ...
        {editingName && !isReadOnly ? (
          ...
          <button
            onClick={isReadOnly ? undefined : startEditName}
            className={cn(
              'flex items-center gap-1.5 text-left flex-1 min-w-0',
              !isReadOnly && 'group',
            )}
          >
              ...
              <DropdownMenuContent align="end" className="w-40">
                {FORMATS.map((f) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      'text-xs',
                      deck.format === f.value && 'text-accent font-medium',
                    )}
                  >
                    {f.label}
                  </DropdownMenuItem>
      ...
      {(deck.commander_name || deck.companion_name || !isReadOnly) && (
        <div className="px-4 pb-2 flex items-center gap-2 flex-wrap">
          ...
          {!isReadOnly && (
            ...
            >
              {descriptionInput
                ? t('deckEditor.editNotes')
                : t('deckEditor.addNotesButton')}
            </button>
          )}
        </div>
      )}
      ...
      {/* Tag Editor */}
      {!isReadOnly && <DeckTagEditor deckId={id!} />}
    </div>
  );
    ...
    <div className="flex items-center gap-1.5 px-4 py-2 border-b border-border bg-card/30">
      <div className="flex items-center gap-0.5 p-0.5 bg-secondary/50 rounded-md">
        {(
          [
            {
              mode: 'list' as DeckViewMode,
              icon: List,
              label: t('deckEditor.view.list'),
            },
            {
              mode: 'visual' as DeckViewMode,
              icon: LayoutGrid,
              label: t('deckEditor.view.visual'),
            },
            {
              mode: 'pile' as DeckViewMode,
              icon: Columns3,
              label: t('deckEditor.view.pile'),
            },
          ] as const
        ...
        <DropdownMenuTrigger asChild>
          <button className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors px-1.5 py-1 rounded hover:bg-secondary/50">
            ...
            <span className="hidden sm:inline">
              {t(`deckEditor.sort.${deckSortMode}`)}
            </span>
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="w-36">
          {(
            ...
            <DropdownMenuItem
              ...
              className={cn(
                'text-xs capitalize',
                deckSortMode === s && 'text-accent font-medium',
              )}
            >
              {t(`deckEditor.sort.${s}`)}
            </DropdownMenuItem>
        ...
        <div className="flex items-center gap-0.5">
          <button
            onClick={() =>
              undoRedo.undo().then((a) => {
                if (a)
                  toast({
                    title: t('deckEditor.undoAction').replace(
                      '{label}',
                      a.label,
                    ),
                  });
          ...
          <button
            onClick={() =>
              undoRedo.redo().then((a) => {
                if (a)
                  toast({
                    title: t('deckEditor.redoAction').replace(
                      '{label}',
                      a.label,
                    ),
                  });
          ...
          {(undoRedo.canUndo || undoRedo.canRedo) && (
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="start"
                className="w-56 max-h-72 overflow-y-auto bg-popover z-50"
              >
      ...
      {!isMobile && (
        <button
          ...
          title={
            previewOpen
              ? t('deckEditor.hidePreview')
              : t('deckEditor.showPreview')
          }
        >
          {previewOpen ? (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
      {grouped.map(([category, catCards]: [string, DeckCard[]]) => (
        <CategorySection
          ...
          onMoveToSideboard={(cardId, toSb) =>
            handleMoveToSideboard(cardId, toSb)
          }
          onMoveToMaybeboard={handleMoveToMaybeboard}
          scryfallCache={scryfallCacheRef}
          onChangePrinting={(cardId, p) =>
  ...
  const deckListContent = (
    <div className="flex-1 overflow-y-auto">
      <div
        className={cn(
          deckViewMode !== 'pile' ? 'p-3 space-y-1' : 'overflow-x-auto',
        )}
      >
        {cardsLoading ? (
          ...
          <div className="flex items-center justify-center py-16 text-muted-foreground text-sm">
            <p className="text-center">
              {isReadOnly
                ? t('deckEditor.emptyDeckReadOnly')
                : t('deckEditor.emptyDeck')}
            </p>
          </div>
        ) : deckViewMode === 'visual' ? (
          <VisualCardGrid
            cards={
              deckSortMode === 'category' ? mainboardCards : sortedMainboard
            }
            scryfallCache={scryfallCacheRef}
            onSelectCard={handleSelectCard}
            selectedCardId={selectedCardId}
...
/** Inline tag editor for the deck header. */
function DeckTagEditor({ deckId }: { deckId: string }) {
  const { t } = useTranslation();
  ...
  const suggestions = popularTags
    .filter(
      (pt) =>
        pt.includes(input.toLowerCase()) && !tags.some((t) => t.tag === pt),
    )
    .slice(0, 5);
  ...
  return (
    <div className="px-4 pb-2 space-y-1.5">
      ...
      {tags.length >= 5 && (
        <p className="text-[10px] text-muted-foreground">{t('deck.maxTags')}</p>
      )}
    </div>
  );
}
