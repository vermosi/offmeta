...
const Index = () => {
  ...
  return <div className="min-h-screen min-h-[100dvh] flex flex-col bg-background">
      {/* Minimal header */}
      ...
      <main className="flex-1 px-3 sm:px-4 py-4 sm:py-8 max-w-4xl mx-auto w-full safe-bottom">
        <div className="space-y-4 sm:space-y-8">
          ...
          {cards.length > 0 ? <>
              ...
              {/* Pagination */}
              {(hasMore || currentPage > 1) && <div className="flex items-center justify-center gap-2 sm:gap-3 pt-4 pb-2">
                  <Button variant="outline" size="sm" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1 || isSearching} className="gap-1.5 h-10 px-3 sm:px-4">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handlePageChange(currentPage + 1)} disabled={!hasMore || isSearching} className="gap-1.5 h-10 px-3 sm:px-4">
                    <span className="hidden sm:inline">Next</span>
                    <ChevronRight className="h-4 w-4" />
                  </Button>
                </div>}
            </> : isSearching ? <div className="flex flex-col items-center justify-center py-16 sm:py-20">
              <Loader2 className="h-6 w-6 sm:h-8 sm:w-8 text-muted-foreground animate-spin" />
              <p className="mt-3 sm:mt-4 text-xs sm:text-sm text-muted-foreground">Searching...</p>
