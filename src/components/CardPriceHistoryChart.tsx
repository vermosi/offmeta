...
export function CardPriceHistoryChart({
    ...
    () =>
      SERIES.filter(
        (series) =>
          ...
          snapshots.some((snapshot) => series.pick(snapshot) != null),
      ),
    [hidden, snapshots],
  );
  ...
  const scale = useMemo(() => {
    const values = visibleSeries.flatMap((series) =>
      snapshots
        .map((snapshot) => series.pick(snapshot))
    ...
    const x = (index: number) =>
      PAD.left + (index / (snapshots.length - 1)) * plotW;
  ...
  const handlePointer = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    const index = Math.round(
      Math.min(1, Math.max(0, ratio)) * (snapshots.length - 1),
    );
    setHoverIndex(index);
  };
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <p className="py-10 text-center text-xs text-muted-foreground">
          {t(
            'priceHistory.stillUploading',
            'Still uploading price data — check back soon.',
          )}
        </p>
      ) : (
        <svg
