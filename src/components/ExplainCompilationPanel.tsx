    ...
    <section className="w-full max-w-3xl mx-auto rounded-2xl border border-border bg-card/60 p-4 sm:p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {intent.deterministicQuery && (
          <span className="text-[10px] text-muted-foreground">
            Deterministic base:{' '}
            <span className="font-mono">{intent.deterministicQuery}</span>
          </span>
        )}
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1">
          ...
          <p
            className={cn('text-sm', !colorSummary && 'text-muted-foreground')}
          >
            {colorSummary || 'None'}
          </p>
        </div>
        ...
        <div className="space-y-1 sm:col-span-2">
          ...
          <p
            className={cn(
              'text-sm',
              intent.tags.length === 0 && 'text-muted-foreground',
            )}
          >
        ...
        <div className="space-y-1 sm:col-span-2">
          ...
          <p
            className={cn(
              ...
              intent.oraclePatterns.length === 0 && 'text-muted-foreground',
            )}
          >
            {intent.oraclePatterns.length
