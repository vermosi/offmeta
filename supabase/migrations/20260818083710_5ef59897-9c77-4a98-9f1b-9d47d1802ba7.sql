...
BEGIN
  LOOP
    CREATE TEMP TABLE _ontology_target AS
      ...
      FROM public.cards c
      WHERE v_last_oracle IS NULL OR c.oracle_id::text > v_last_oracle
      ORDER BY c.oracle_id
      LIMIT v_size;

    SELECT count(*), max(oracle_id::text) INTO v_batch_cards, v_last_oracle FROM _ontology_target;
