CREATE OR REPLACE FUNCTION admin_api.table_row_estimate(_table regclass)
 RETURNS bigint
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
  SELECT GREATEST(COALESCE(c.reltuples, 0), 0)::bigint FROM pg_class c WHERE c.oid = _table;
$function$;
...
BEGIN
  ...
  -- Row counts use planner estimates: exact counts on multi-million row tables
  -- (price_snapshots) blew past the 15s statement timeout and failed the panel.
