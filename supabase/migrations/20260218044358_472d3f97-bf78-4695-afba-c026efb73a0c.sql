...
-- Fix: Add admin role check to get_search_analytics RPC (SECURITY DEFINER bypass)
-- The function is callable directly by any authenticated user without authorization.
-- Adding has_role() check ensures only admins can execute it.
...
DECLARE
  ...
  coverage_data jsonb;
BEGIN
  -- Enforce admin-only access: reject non-admin callers even if called directly
