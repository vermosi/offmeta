...
Deno.serve(async (req: Request) => {
  ...
  // Auth: service role or pipeline key (for cron)
  ...
  try {
    ...
    const { data: candidates } = await supabase.rpc('get_promotion_candidates', {
    ...
    if (newQueries.length === 0) {
      logEvent('info', 'auto_seo_no_candidates', { existingCount: existingQueries.size });
      return new Response(
        JSON.stringify({ message: 'No new candidates found', existingPages: existingQueries.size }),
        { status: 200, headers },
      );
    }
    ...
    for (const query of newQueries) {
      try {
        ...
        results.push({ query, status: 'error', error: err instanceof Error ? err.message : 'Unknown' });
      }
    ...
    logEvent('info', 'auto_seo_complete', { total: newQueries.length, succeeded });
