  ...
  try {
    ...
    try {
      ...
      if (body?.queries && Array.isArray(body.queries)) {
        customQueries = body.queries.filter((q: unknown) => typeof q === 'string' && q.length > 0);
      }
    } catch {
      // No body or invalid JSON - use defaults only
    }

    const queriesToWarm = customQueries.length > 0 ? customQueries : COMMON_QUERIES;
    ...
    return new Response(JSON.stringify({
      ...
      ...(results.errors.length > 0 && results.errors.length <= 10 
        ...
        : results.errors.length > 10 
          ? { errors: results.errors.slice(0, 10), moreErrors: results.errors.length - 10 }
          : {})
    }), {
