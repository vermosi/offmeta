...
Deno.test(
  'Local DB: batch card lookup returns results with metadata',
  async () => {
    ...
    console.log(
      `✅ Found ${items.length}/${TEST_CARDS.length} cards in local DB`,
    );
...
Deno.test('Local DB: batch query completes in under 500ms', async () => {
  ...
  assertEquals(
    elapsed < 500,
    true,
    `Batch query took ${elapsed.toFixed(0)}ms, should be < 500ms`,
  );
});
...
Deno.test(
  'Local DB: cards data is recent (updated within 14 days)',
  async () => {
    ...
    const daysSinceUpdate =
      (Date.now() - latestUpdate.getTime()) / (1000 * 60 * 60 * 24);
    console.log(
      `📊 Latest card update: ${latestUpdate.toISOString()} (${daysSinceUpdate.toFixed(1)} days ago)`,
    );
    assertEquals(
      daysSinceUpdate < 14,
      true,
      `Cards data is ${daysSinceUpdate.toFixed(1)} days stale, should be < 14`,
    );
  },
);
...
Deno.test(
  'generate-seo-page: rejects queries shorter than 3 chars',
  async () => {
...
Deno.test(
  'generate-seo-page: rejects queries longer than 200 chars',
  async () => {
...
Deno.test('generate-seo-page: returns 409 for duplicate slug', async () => {
  ...
  console.log(
    `📊 Duplicate test: status=${res.status}, body=${JSON.stringify(body).slice(0, 100)}`,
  );
