...
const SIGNATURES: readonly Signature[] = [
  ...
  { tag: 'mill', test: /\bmills?\b/i },
  {
    tag: 'reanimate',
    test:
      /return\s+(?:target|up to \w+ target)?\s*(?:creature|permanent)\s+cards?\s+from\s+(?:your|a)\s+graveyard\s+to\s+the\s+battlefield/i,
  },
  {
    tag: 'recursion',
    test: /return\s+.*\bfrom\s+(?:your|a)\s+graveyard\s+to\s+your\s+hand/i,
  },
  { tag: 'cast-from-graveyard', test: /(?:you may )?cast\s+.*\bfrom\s+your\s+graveyard/i },
  { tag: 'cheat-into-play', test: /put\s+(?:it|that card|them)\s+onto\s+the\s+battlefield/i },

  // Card advantage / selection
  { tag: 'wheel', test: /each player (?:discards|shuffles).*draws\s+seven|discards? their hand.*draws? seven/i },
  { tag: 'loot', test: /draws?\s+.*\bcards?\b.*then\s+discards?/i },
  { tag: 'draw-engine', test: /whenever\s+.*\bdraw\s+a\s+card\b|at the beginning of your\s+\w+\s+step,\s+draw/i },
  ...
  { tag: 'land-ramp', test: /search your library for (?:a|up to \w+)\s+(?:basic\s+)?land card.*(?:battlefield|onto)/i },
  ...
  // Interaction
  { tag: 'counterspell', test: /counter target (?:spell|creature spell|noncreature spell)/i },
  { tag: 'sweeper', test: /destroy all|exile all|each creature gets -\d+\/-\d+|all creatures get -\d/i },
  {
    tag: 'spot-removal',
  ...
  { tag: 'copy', test: /(?:copy|as a copy) of (?:target|that) (?:spell|creature|permanent)/i },
  { tag: 'tax', test: /costs? \{?\d?\w?\}? more to cast|unless (?:that player|they) pays/i },
  { tag: 'pillowfort', test: /can't attack you|attacks? you .*unless/i },
  { tag: 'protects-creature', test: /(?:gains?|has) (?:hexproof|indestructible|protection)/i },
];
